...
int poss_num_of_wins(int turn_char)
{
    ...
    for(int i=0;i<3;i++)
    {
        ...
        if(x == val || y==val )
        {
          for(int j=0;j<3;j++)
          {
            if(board[(i*3) + j] == 2 && x==val){num++;}
            if(board[i + (3*j)] == 2 && y==val){num++;}
          }
        }
    }
    ...
    if(z1 == val || z2 == val)
    {
      for(int j=0;j<3;j++)
      {
        if(board[4*j] == 2 && z1 == val){num++;}
        if(board[2 + (2*j)] == 2 && z2 == val){num++;}
      }
    }
    return num;
}
...
void comp_move(int turn)
{
  switch (turn)
  {
    ...
    default:
      ...
      if(turn == 5)
      {
        board[6] == 2 ? board[6] = 5 : board[2] = 5;return;
      }
      else
      {
          board[make2()] = 5;return;
      }
